...
const { PhysicsIntegrator } = require('./physics_engine.cjs');
...
const outputObjPath = path.join(scratchDir, 'four_cylinder_engine.obj');
...
const phys = new PhysicsIntegrator(3000, 0.044, 0.150);
const simResult = phys.runSimulation(0.020, 0.002);
...
const outputBmpPath = path.join(scratchDir, 'engine_pbr_preview.bmp');
